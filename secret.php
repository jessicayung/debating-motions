  ...
  <body>
    ...
    <div class="container">
      ...
      <?php
        ...
        $sql = "ALTER TABLE Motions ORDER BY Date DESC, Round ASC UNION SELECT Round, Motion, Tournament FROM Motions ORDER BY ID ASC";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
                 echo "<table class=\"table table-striped\"><tr><th>Tournament</th><th>Round</th><th>Motion</th></tr>";
                 // output data of each row
                 while($row = $result->fetch_assoc()) {
                     echo "<tr><td>" . $row["Tournament"]. "</td><td>" . $row["Round"]. "</td><td>". $row["Motion"]. "</td></tr>";
                 }
                 echo "</table>";
        } else {
                 echo "0 results";
